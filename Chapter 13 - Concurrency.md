...
- [동기화 구간은 가능한한 작게 만들라](#8)
- [제대로 종료되는하는 코드는 작성하기 어렵다](#9)
- [스레드를 포함하는 코드 테스트하기](#10)
 - [일시적이라고 판단되는 문제를 스레드 관련 이슈의 후보로 생각하라](#10-1)
 - [스레드와 관련없는 코드를 작동하게 하라](#10-2)
 - [스레드와 연관될 코드를 Pluggable하게 만들어라](#10-3)
 - [스레드와 연관될 코드를 Tunable하게 만들어라](#10-4)
 - [프로세서의 수 보다 많은 수의 스레드를 실행해 보라](#10-5)
 - [다른 플렛폼에서 실행해 보라](#10-6)
 - [문제가 발생하도록 조작해 보라](#10-7)
   - [직접 조작](#10-7-1)
    - [자동화된 조작](#10-7-2)
- [결론](#11)
...
<a name="8"></a>
## 동기화 구간은 가능한한 작게 만들라 ##
Synchronized로 수행되는 잠금은 딜레이와 오버해드를 만들기 때문에 "비싼 수행"으로 간주되며 가능한한 작게 만들어야 한다. 반면 critical section은 꼭 보호되어야 한다.

**추천**: *동기화된 영역은 최대한 작게 만들어라.*

<a name="9"></a>
## 제대로 종료되는 코드는 작성하기 어렵다 ##
...
<a name="10-1"></a>
#### 일시적이라고 판단되는 문제를 스레드 관련 이슈의 후보로 생각하라 ####
멀티 스레드 코드는 일반적으로 발생할 리 없어 보이는 문제를 발생시킨다. 저자를 포함한 대부분의 개발자는 이러한 문제를 직관적으로 파악하지 못한다. 또한 이는 매우 드물게 발생해 개발자들을 좌절하게 만든다. 그래서 개발자들은 이러한 문제들을 우주선(宇宙線), 하드웨어 버그, 혹은 이러한 류의 "one-off"로 치부한다. 제일 좋은 방향은 "one-off"는 없다고 판단하는 것이다. 이러한 one-off들이 무시될 수록 더 많은 코드들이 이미 문제가 있는 시스템에 추가되게 될 뿐이다.
...
- 단일 스레드, 여러 스레드 환경에서
- 실제 사용될 객체 혹은 Test Double<sup> [6](#fn6)</sup>과 상호작용할 수 있는 스레드 코드
- 수행 속도를 조절할 수 있는 Test Double
- 지정된 횟수만큼 반복 수행할 수 있게
...
<a name="10-5"></a>
#### 프로세서의 수 보다 많은 수의 스레드를 실행해 보라 ####
시스템이 작업을 전환할 때에도 문제는 발생한다. 작업 전환을 빈번히 발생하게 하기 위해 프로세서 수보다 많은 스레드를 실행해 보라. 작업 전환이 잦을 수록 빠뜨린 critical section이나 dead lock을 찾을 확률이 높아지게 된다.

<a name="10-6"></a>
#### 다른 플렛폼에서 실행해 보라 ####
우리(저자)는 2007년 중순 concurrent 프로그래밍 강좌를 개발했다. 강좌의 개발은 OSX에서 진행되었으며 시연은 VM상의 Windows XP에서 진행되었다. 하지만 실패를 시연하기 위해 작성된 테스트는 OSX에서는 자주 발생했지만 Windows XP에서는 발생하지 않았다.
...
혹은 이와 비슷한 작업을 수행해 주는 IBM에서 개발한 ConTest라는 툴도 있다. 이는 수행시마다 다른 순서로 스레드를 실행하게 만들어 줌으로써 문제를 발견할 확률을 극적으로 높여주게 된다.
