...
네 가지 규칙을 따르면 코드 구조와 설계를 파악하기 쉬워진다면? 그래서 SRP(Single Responsibility Principle, 단일 원칙)나 DIP(Dependency Inversion Principle, 의존 관계 역전 원칙)와 같은 원칙을 적용하기 쉬워진다면? 네 가지 규칙이 우수한 설계의 창발성을 촉진한다면?
